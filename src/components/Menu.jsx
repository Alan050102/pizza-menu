...
export default function Menu() {
    // ir creando el estado con las piizas
    ...
    // Obteniendo datos de localStorage si existen, este hook es lanzado cuando se construye el componente
    useEffect(() => {
    ...
    // Funcion para borrar las pizzas y actualizarlo al estado
    const deletePizza = (pizzaName) => setPizzas(prev => prev.filter((pizza) => pizza.name !== pizzaName))
    return (
        <main className="menu">
            <h2>Our menu</h2>
            {pizzas.length > 0 ?
                // *<> </>: Esta etiqueta se conoce como fragment y no se renderiza en la aplicación nos sirve como contenedor padre pero sin ser renderizada en html
                (<>
                    <p>
                    ...
                    <p style={{marginBottom: "17rem"}}>We're still working on our menu, Please come back later.</p>
                )
            }
        </main>
    );
